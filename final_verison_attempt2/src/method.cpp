...
void Method::separateThread() {
  ...
  Generate_Turtlebots(3);
  std::vector<std::thread> threads;
  int numTurtlebot = turtlebots.size();
  ...
  std::vector<double> time_offset(turtlebots.size(), 0.0);
  std::vector<std::vector<geometry_msgs::Point>> trajectory(numTurtlebot);
  std::vector<std::vector<geometry_msgs::Point>> trajectoryTemp(numTurtlebot);
  ...
  if (Synchronous){
    for (int i = 0; i < largestSize; i++){
      ...
      std::cout << "publishing goals" << std::endl;

      // visualization_msgs::MarkerArray tempory = publishMarkers(combinedPoints, marker_pub);

      for(int v = 0; v < time_offset.size(); v++){
        time_offset.at(v) = 0;
      }

      trajectoryTemp = trajectory;
      if (path_checker_function(trajectoryTemp, time_offset)){
        trajectory = trajectoryTemp;
      }
      ...
      // tempory = publishMarkers(combinedPoints, marker_pub);
      ...
      for (size_t j = 0; j < turtlebots.size(); j++) {
        threads.emplace_back(&Method::Function, std::ref(*this), trajectory.at(j), std::ref(turtlebots.at(j)), std::ref(Turtle_Controllers.at(j)), std::ref(time_offset.at(j)));
...
// GENERATING AND FILL PARAMETERS
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void Method::Generate_Turtlebots(int x){
  //Create turtlebot section
  /////////////////////////////////////////////////////////
  ros::NodeHandle nh_0;
  DefaultTurtleBot* tb0 =  new DefaultTurtleBot("", nh_0);
  ros::NodeHandle nh_1;
  ...
  int x_turtlebots;
  std::cout << "HoW many turtlebots would you like to use" << std::endl;
  std::cin >> x_turtlebots;

  switch (x_turtlebots) {
    case 1:
     
      
      turtlebots.push_back(tb0);
      break;
    case 2:
