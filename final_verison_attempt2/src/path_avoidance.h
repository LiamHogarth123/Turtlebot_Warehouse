...
public:
    ...
    void Update_paths(std::vector<std::vector<geometry_msgs::Point>> input, std::vector<double> time_offset);
...
private:
    ...
    std::vector<double> time_offset_;

    std::vector<TurtleBotState> interpolatePositions(const std::vector<geometry_msgs::Point>& waypoints, double average_speed, double wait_times);
