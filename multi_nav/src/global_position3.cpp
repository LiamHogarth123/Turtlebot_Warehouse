...
class GetGlobalPosition3
{
public:
    GetGlobalPosition3()
    {
        ros::NodeHandle nh;
        nh.getParam("/global_position3/tb0", tb3_0);
        nh.getParam("/global_position3/tb1", tb3_1);
        nh.getParam("/global_position3/tb2", tb3_2);
        ...
        pose_marker_pub_3 = nh.advertise<visualization_msgs::Marker>("/" + tb3_2 + "/marker/position", 10);
    ...
    void getPosition()
    {
        ...
        while (ros::ok())
        {
            ...
            success = getTransform(tf_listener, tb3_2 + "/base_footprint", transform);
            if (success)
            {
                geometry_msgs::Point point;
                point.x = transform.getOrigin().x();
                point.y = transform.getOrigin().y();
                point.z = 0.0;
                pose_points_3.points.push_back(point);
                pose_marker_pub_3.publish(pose_points_3);
            }
...
private:
    ...
    ros::Publisher pose_marker_pub_1, pose_marker_pub_2, pose_marker_pub_3;
    visualization_msgs::Marker pose_points_1, pose_points_2, pose_points_3;
    ...
    void position_marker_3()
    {
        pose_points_3.header.frame_id = "map";
        pose_points_3.header.stamp = ros::Time::now();
        pose_points_3.ns = "points";
        pose_points_3.id = 0;
        pose_points_3.type = visualization_msgs::Marker::POINTS;
        pose_points_3.action = visualization_msgs::Marker::ADD;
        pose_points_3.pose.orientation.w = 1.0;
        pose_points_3.scale.x = 0.3;
        pose_points_3.scale.y = 0.3;
        pose_points_3.color = getColor(0.0, 0.0, 1.0, 1.0);
        pose_points_3.lifetime = ros::Duration();
    }
...
{
    ros::init(argc, argv, "global_position3");

    GetGlobalPosition3 global_position;
