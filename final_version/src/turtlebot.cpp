...
DefaultTurtleBot::DefaultTurtleBot(const std::string& name, const ros::NodeHandle& nh)  : namespace_(name){  // Create publishers and subscribers with namespaced topics/services
  ...
  sub5_ = nh_.subscribe(namespace_ + "/boundary/detection", 10, &DefaultTurtleBot::boundaryCallback,this); 

  sub6_ = nh_.subscribe(namespace_ + "/markers/info", 10, &DefaultTurtleBot::tagCallback,this); 

}

bool DefaultTurtleBot::IsFunctional(){
  return true;
}
